...
This script reuses EXACTLY the same RTDS measuring pipeline as the 4x4 GNC script:
- 4 experiments per frequency (inj1 d/q, inj2 d/q)
- builds V_exc (4x4) and I_exc (4x4)
- computes measured Y_tot(4x4) = I_exc * inv(V_exc)

Then it performs EVD diagnostics on the CLOSED-LOOP IMPEDANCE:
    Z_cl(jw) = (Y_tot(jw))^{-1}
...
Quality gating:
- same metrics as 4x4 code (harmonics, drift, SNR, inversion-flag)
...
if len(freqs_curves) < 2:
    raise RuntimeError("Too few curve-valid points for EVD plots.")

lam_branches = track_eig_branches({f: eigvals_by_f[f] for f in freqs_curves}, freqs_curves)

# ── b) |lambda_max| vs frequency ─────────────────────────────────────────────
...
me_lm = max(1, len(freqs_curves) // 15)
ax.semilogy(freqs_curves, lammax_curve,
            '-', linewidth=1.2, color='C0',
            marker='s', markersize=4, markerfacecolor='none',
            markeredgecolor='C0', markeredgewidth=1.0,
            markevery=me_lm, label=r'$|\lambda_{\max}|$')
ax.set_xlim(freqs_curves[0], freqs_curves[-1])
_freq_ticks(ax, freqs_curves[0], freqs_curves[-1])
_log_ylim(ax, lammax_curve)
ax.set_xlabel('Frequency (Hz)'); ax.set_ylabel(r'$|\lambda_{\max}|$')
ax.legend(**_LEG_FREQ); ax.grid(True, which='both', **_GRID)
fig.tight_layout()
fig.savefig(_os.path.join(OUT_DIR, 'evd_lammax.svg'), **_SAVE); plt.show()

print(f"\nCurve-valid: {len(freqs_curves)}/{len(freqs_all)} | f* = {f_star:.2f} Hz | |lambda_max| = {lammax_star:.4e}")

# ── c) Participation factors at f* ────────────────────────────────────────────
if f_star in eigvecs_store:
    lam_fs  = eigvecs_store[f_star]["lam"]
    V_fs    = eigvecs_store[f_star]["V"]
    U_fs    = eigvecs_store[f_star]["U"]
    k_star  = int(np.argmax(np.abs(lam_fs)))
    lam_star = lam_fs[k_star]
    p       = participation_factors(U_fs[:, k_star], V_fs[:, k_star])
    labels  = [f"{PCC1['name']}-d", f"{PCC1['name']}-q",
               f"{PCC2['name']}-d", f"{PCC2['name']}-q"]

    print(f"λ* = {lam_star.real:+.4e}{lam_star.imag:+.4e}j  |λ*| = {abs(lam_star):.4e}")
    for i, lab in enumerate(labels):
        print(f"  p[{lab}] = {p[i]:.4f}")

    fig, ax = plt.subplots(figsize=(4.5, 3.5))
    ax.bar(labels, p, color='C0', edgecolor='black', linewidth=0.8)
    ax.set_xlabel('Channel'); ax.set_ylabel('Participation (normalized)')
    ax.set_ylim(0, min(1.0, float(np.max(p)) * 1.15))
    ax.grid(True, axis='y', **_GRID)
    fig.tight_layout()
    fig.savefig(_os.path.join(OUT_DIR, 'evd_participation.svg'), **_SAVE); plt.show()
else:
    print("[WARN] No stored eigvecs at f* — participation plot skipped.")
