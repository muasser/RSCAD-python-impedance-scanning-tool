# -*- coding: utf-8 -*-
"""
GNC MIMO Stability Tool v24 — RSMM at critical frequency f*
==============================================================================
TWO PCCs (two injections) -> 4x4 dq-domain

CHANGES vs v23:
  - [FIX] Sweep parameter changed from KpPLL to K_Margin_Inner.
          RSCAD slider names (GF, GF1, R, L) are UNCHANGED.
  - [FIX] Vbase_LL = 400e3, Sbase_3ph = 800e6 (model 2: 400kV/800MVA).
  - [FIX] PCC paths updated with Substep|MMC_DCS1 prefix.
  - [FIX] All plot labels/titles/prints now show K_Margin_Inner.
  - [KEEP] f* = auto-determined as frequency with highest RSMM std dev.
  - [KEEP] GF_PER_SCR summary: RSMM(f*) vs K_Margin_Inner per SCR,
           different colours per case — f* logic (NOT min RSMM).


FOUR SWEEP MODES (set via SWEEP_MODE):
  "GF"         — K_Margin_Inner sweep at one fixed SCR case (ACTIVE_CASE).
  "SCR"        — SCR sensitivity at fixed K_Margin_Inner.
  "XR"         — X/R sensitivity at fixed K_Margin_Inner and SCR.
  "GF_PER_SCR" — K_Margin_Inner sweep per SCR case.
...
# Placeholder; actual value is only determined AFTER the sweep.
F_CRIT_FIXED = None

# --------------------------------------------------
# CRITICAL FREQUENCY — automatically determined
# as the frequency with the largest RSMM variation
# across all sweep runs (highest std dev over KpPLL).
# --------------------------------------------------

# GF slider -> St2Gu1KpPLL (sweep), nominal = 5.0
GF_SWEEP_VALUES = [0.5, 1.0, 1.5, 2.0]

# Only needed when SWEEP_MODE = "XR"
XR_SWEEP_VALUES = [5.0, 7.5, 10.0, 15.0, 20.0]

# GF1 slider -> St2Gu1TiPLL fixed at nominaal 0.01
KPVDC_FIXED = 0.01
...
print(f"GF  slider  -> K_Margin_Inner sweep : {GF_SWEEP_VALUES}")
print(f"GF1 slider  -> St2Gu1TiPLL vast    : {KPVDC_FIXED}")
print("f*          : auto-determined after sweep (highest RSMM std over runs)")
print(f"GF_BASELINE : {GF_BASELINE}  (nominale K_Margin_Inner)")
...
def get_timeseries_consistent(case, sig_handles, timeout_s=30.0,
    ...
    while time.time() - t_start < timeout_s:
        ...
        if ok:
            ...
            if stable_count >= 1:
                return t_ref, data
    raise RuntimeError("Could not obtain a consistent time series (timeout).")
...
def set_gf_slider(Hinj, gf_val):
    ...
    if Hinj["inj1"]["GF1"] is not None:
        ...
        print(f"  [GF] K_Margin_Inner (GF) = {gf_val:.4f}  [SWEEP]")
        print(f"  [GF] St2Gu1TiPLL (GF1)  = {KPVDC_FIXED:.4f}  [FIXED]")
        print(f"  [GF] Waiting {GF_SETTLE_S}s for controller to settle...")
    else:
...
# 12) PLOTS
# ==================================================
def plot_eigenloci(eig_results_, freqs, title_suffix):
    all_freqs = sorted(eig_results_.keys())
    if len(all_freqs) < 2:
...
def plot_rsmm_profile(freqs, min_dist_results_, title_suffix):
...
def plot_single_sweep_summary(sweep_results, f_star, xlabel, title):
    ...
    plt.axvline(GF_BASELINE, color="green", linestyle=":", linewidth=1.5,
                label=f"Nominal K_Margin_Inner = {GF_BASELINE}")
...
def plot_gf_per_scr_summary(gf_per_scr_results, f_star):
    ...
    plt.axvline(GF_BASELINE, color="green", linestyle=":", linewidth=1.2,
                label=f"Nominal K_Margin_Inner = {GF_BASELINE}")
    plt.xlabel("K_Margin_Inner — inner current controller gain")
    plt.ylabel(f"RSMM(f*={f_star:.1f} Hz) = sigma_min(I+L)")
    plt.title(f"K_Margin_Inner sweep per SCR — RSMM(f*={f_star:.1f} Hz) vs K_Margin_Inner\n"
...
def print_gf_per_scr_table(gf_per_scr_results, f_star):
    print(f"\n{'='*65}")
    print(f"  RESULTS TABLE — K_Margin_Inner vs RSMM(f*={f_star:.1f} Hz) per SCR")
    print(f"  (St2Gu1TiPLL fixed at {KPVDC_FIXED})")
    print(f"{'='*65}")
    print(f"  {'Case':>5}  {'SCR':>5}  {'K_Mar.*':>8}  {'RSMM(f*)':>10}  {'Verdict':>10}")
    print(f"  {'-'*5}  {'-'*5}  {'-'*8}  {'-'*10}  {'-'*10}")
    for case_key, points in gf_per_scr_results.items():
        scr = GRID_CASES[case_key]["SCR"]
        if not points:
...
def build_sweep_plan():
    ...
    if SWEEP_MODE == "GF":
        ...
        print(f"\n{'='*60}")
        print(f"  MODE: K_Margin_Inner sweep at fixed grid condition")
        print(f"  Case   : {ACTIVE_CASE}  (SCR={params['SCR']:.1f}, X/R={params['XR']:.0f})")
        print(f"  R={params['R']:.3f} Ohm  L={params['L']:.3f} H")
        print(f"  K_Margin_Inner sweep : {GF_SWEEP_VALUES}")
        ...
        print(f"\n{'='*60}")
        print(f"  MODE: SCR sensitivity at fixed K_Margin_Inner={GF_BASELINE}")
        ...
        print(f"\n{'='*60}")
        print(f"  MODE: X/R sensitivity (SCR={base_scr:.1f} fixed, K_Margin_Inner={GF_BASELINE})")
        print(f"  KpVdc fixed: {KPVDC_FIXED}")
        ...
        print(f"\n{'='*60}")
        print(f"  MODE: K_Margin_Inner sweep per SCR case")
        print(f"  Cases               : {cases}")
        print(f"  K_Margin_Inner sweep: {GF_SWEEP_VALUES}")
        print(f"  TiPLL fixed         : {KPVDC_FIXED}")
        print("  f* critical : auto-determined after sweep")
...
with rtds.rscadfx.remote_connection() as app:
    ...
    try:
        ...
        for run_idx, (param_label, param_val, case_key) in enumerate(sweep_plan):
            ...
            if SWEEP_MODE in ("GF", "GF_PER_SCR"):
                print(f"  K_Margin_Inner (GF)  = {param_val:.4f}  [SWEEP]")
            ...
            print(f"  Case={case_key}  SCR={params['SCR']:.1f}  "
                  + (f"K_Margin_Inner={param_val:.4f}" if SWEEP_MODE in
                     ("GF", "GF_PER_SCR") else ""))
            ...
            title_sfx = (f"Case {case_key} SCR={params['SCR']:.1f}  "
                         + (f"K_Margin_Inner={param_val:.4f}  TiPLL={KPVDC_FIXED}"
                            if SWEEP_MODE in ("GF", "GF_PER_SCR")
                            else f"K_Margin_Inner={GF_BASELINE}  TiPLL={KPVDC_FIXED}"))
...
def plot_fcrit_motivation(std_per_freq, f_star):
    if not std_per_freq:
        return
    ...
    plt.xlabel("Frequency [Hz]")
    plt.ylabel("Std dev of RSMM over K_Margin_Inner sweep")
    plt.title("Basis for f* selection — RSMM variation per frequency\n"
              "f* is the frequency most sensitive to K_Margin_Inner changes")
    plt.legend()
    ...
    if SWEEP_MODE == "GF_PER_SCR":
        ...
        if len(sweep_results) >= 2:
            plot_single_sweep_summary(
                sweep_results,
                f_star,
                xlabel="K_Margin_Inner — inner current controller gain",
                title=(f"K_Margin_Inner sweep — Case {ACTIVE_CASE}  "
