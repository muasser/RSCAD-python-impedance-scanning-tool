...
def settle_time(f_hz):
    ...
    return float(max(0.8, min(2.0, 8.0 / max(f_hz, 2.0))))
...
fig.tight_layout()
fig.savefig('gnc_eigenloci_raw.svg', **_SAVE); plt.show()
...
ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
fig.tight_layout()
fig.savefig('gnc_eigenloci_raw_zoom.svg', **_SAVE); plt.show()
...
ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
fig.tight_layout()
fig.savefig('gnc_nyquist_det_raw.svg', **_SAVE); plt.show()
...
if VF_ENABLE:
    ...
    if len(freqs_vf_in) < 5:
        ...
        if Y_vf is not None:
            ...
            ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
            fig.tight_layout()
            fig.savefig('gnc_eigenloci_vf.svg', **_SAVE); plt.show()
            ...
            ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
            fig.tight_layout()
            fig.savefig('gnc_eigenloci_vf_zoom.svg', **_SAVE); plt.show()
            ...
            ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
            fig.tight_layout()
            fig.savefig('gnc_nyquist_det_vf.svg', **_SAVE); plt.show()
