...
INJECTIONS = [
    {"name": "inj1", "Finj_slider": "Finj", "d_gain_slider": "d_gain",
     "q_gain_slider": "q_gain", "PB_d": "PB", "PB_q": "PB1"},
    {"name": "inj2", "Finj_slider": "Finj1", "d_gain_slider": "d_gain1",
     "q_gain_slider": "q_gain1", "PB_d": "PB2", "PB_q": "PB3"},
]
...
def settle_time(f_hz):
    f_hz = float(f_hz)
    if is_slow_mode(f_hz): return float(max(3.0, min(7.0, 14.0 / max(f_hz, 0.7))))
    ...
    for col, (inj_name, d_gain, q_gain) in enumerate(experiments):
        ...
        while True:
            try:
                ...
                # Quality on excited port only (original approach)
                ...
                if attempts < 1:
                    attempts += 1; print(f"[RECOVER] col={col} {e} -> HARD RECOVERY (1/1)")
                    case, Hinj, Hpcc = recover_by_reopen(app, case); continue
                raise
...
_LEG_FREQ = dict(loc='best',        frameon=True, edgecolor='black', fancybox=False, framealpha=1.0)
_LEG_CPLX = dict(loc='upper right', frameon=True, edgecolor='black', fancybox=False, framealpha=1.0)
_GRID     = dict(linestyle=':', linewidth=0.5, alpha=0.6)
_SAVE     = dict(format='svg', bbox_inches='tight', pad_inches=0.02)

def _tight(values, margin=0.02):
    lo, hi = float(np.min(values)), float(np.max(values))
    pad = margin * (hi - lo) if hi != lo else 0.1
    return lo - pad, hi + pad

def _freq_ticks(ax, f_lo, f_hi):
    xrange = float(f_hi) - float(f_lo)
    step = 1 if xrange <= 10 else (5 if xrange <= 50 else 10)
    interior = np.arange(np.ceil(float(f_lo) / step) * step,
                         float(f_hi) + 1e-9, step)
    ticks = sorted(set([float(f_lo)] + list(interior) + [float(f_hi)]))
...
def _log_ylim(ax, data):
    lo = 10 ** np.floor(np.log10(float(np.min(data))))
    hi = 10 ** np.ceil(np.log10(float(np.max(data))))
    if lo == hi: hi = lo * 10
    ax.set_ylim(lo, hi)
...
for i, lam in enumerate(lam_branches):
    me = max(1, len(lam) // 15)
    ax.plot(lam.real, lam.imag,
            LINE_STYLES[i % 4], linewidth=1.2, color=f'C{i}',
            marker=MARKERS[i % 4], markersize=4,
            markerfacecolor='none', markeredgecolor=f'C{i}', markeredgewidth=1.0,
            markevery=me, label=fr'$\lambda_{{{i+1}}}$')
ax.scatter([-1], [0], color='red', zorder=5, marker='x', s=60, linewidths=1.5)
all_r = np.concatenate([lam.real for lam in lam_branches] + [np.array([-1.0])])
all_i = np.concatenate([lam.imag for lam in lam_branches] + [np.array([0.0])])
ax.set_xlim(*_tight(all_r)); ax.set_ylim(*_tight(all_i))
_cplx_ticks(ax)
ax.set_xlabel('Real part'); ax.set_ylabel('Imaginary part')
ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
fig.tight_layout()
fig.savefig(_os.path.join(OUT_DIR, 'gnc_4x4_eigenloci_raw.svg'), **_SAVE); plt.show()

# ── b) Eigenloci raw zoom ─────────────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(4.5, 4.5))
for i, lam in enumerate(lam_branches):
    me = max(1, len(lam) // 15)
    ax.plot(lam.real, lam.imag,
...
ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
fig.tight_layout()
fig.savefig(_os.path.join(OUT_DIR, 'gnc_4x4_eigenloci_raw_zoom.svg'), **_SAVE); plt.show()

det_curve = np.array([det_results[f] for f in freqs_curves], dtype=complex)

# ── c) Nyquist det(I+L) raw ───────────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(4.5, 4.5))
me_d = max(1, len(det_curve) // 15)
ax.plot(det_curve.real, det_curve.imag,
        '-', linewidth=1.2, color='C0',
        marker='s', markersize=4, markerfacecolor='none',
        markeredgecolor='C0', markeredgewidth=1.0,
        markevery=me_d, label=r'$\det(I+L)$')
ax.scatter([0], [0], color='red', zorder=5, marker='x', s=60, linewidths=1.5)
all_r_d = np.append(det_curve.real, 0.0); all_i_d = np.append(det_curve.imag, 0.0)
ax.set_xlim(*_tight(all_r_d)); ax.set_ylim(*_tight(all_i_d))
_cplx_ticks(ax)
ax.set_xlabel('Real part'); ax.set_ylabel('Imaginary part')
ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
fig.tight_layout()
fig.savefig(_os.path.join(OUT_DIR, 'gnc_4x4_nyquist_det_raw.svg'), **_SAVE); plt.show()
        ...
        if Y_vf is not None:
            ...
            for i, lam in enumerate(lam_vf):
                ...
                ax.plot(lam.real, lam.imag,
                        ...
                        marker=MARKERS[i % 4], markersize=4,
                        markerfacecolor='none', markeredgecolor=f'C{i}', markeredgewidth=1.0,
                        markevery=me, label=fr'$\lambda_{{{i+1}}}$ VF')
            ax.scatter([-1], [0], color='red', zorder=5, marker='x', s=60, linewidths=1.5)
            all_r_v = np.concatenate([lam.real for lam in lam_vf] + [np.array([-1.0])])
            all_i_v = np.concatenate([lam.imag for lam in lam_vf] + [np.array([0.0])])
            ax.set_xlim(*_tight(all_r_v)); ax.set_ylim(*_tight(all_i_v))
            _cplx_ticks(ax)
            ax.set_xlabel('Real part'); ax.set_ylabel('Imaginary part')
            ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
            fig.tight_layout()
            fig.savefig(_os.path.join(OUT_DIR, 'gnc_4x4_eigenloci_vf.svg'), **_SAVE); plt.show()
            ...
            ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
            fig.tight_layout()
            fig.savefig(_os.path.join(OUT_DIR, 'gnc_4x4_eigenloci_vf_zoom.svg'), **_SAVE); plt.show()

            det_vf_c = np.array([det_vf[f] for f in fvs], dtype=complex)

            # ── g) Nyquist det(I+L) VF ────────────────────────────────────────
            fig, ax = plt.subplots(figsize=(4.5, 4.5))
            me_dv = max(1, len(det_vf_c) // 15)
            ax.plot(det_vf_c.real, det_vf_c.imag,
                    '-', linewidth=1.2, color='C0',
                    marker='s', markersize=4, markerfacecolor='none',
                    markeredgecolor='C0', markeredgewidth=1.0,
                    markevery=me_dv, label=r'$\det(I+L)$ VF')
            ax.scatter([0], [0], color='red', zorder=5, marker='x', s=60, linewidths=1.5)
            all_r_dv = np.append(det_vf_c.real, 0.0); all_i_dv = np.append(det_vf_c.imag, 0.0)
            ax.set_xlim(*_tight(all_r_dv)); ax.set_ylim(*_tight(all_i_dv))
            _cplx_ticks(ax)
            ax.set_xlabel('Real part'); ax.set_ylabel('Imaginary part')
            ax.legend(**_LEG_CPLX); ax.grid(True, **_GRID)
            fig.tight_layout()
            fig.savefig(_os.path.join(OUT_DIR, 'gnc_4x4_nyquist_det_vf.svg'), **_SAVE); plt.show()
...
# Save after RTDS disconnected
try:
    np.savez("gnc_4x4_results.npz",
    ...
    print("Saved: gnc_4x4_results.npz")
except Exception as e:
